...
#				  TF-IDF scores as keys, list of word as values
#---------------------------------------------------------------------------------
...
def buildQuery(sentences, TF_IDF_w, n):
    ...
    # NOTE: to avoid out of range error
	n = min(n, len(scores))
...
#				  summary_length, desired number of sents for the summary
# 									NOTE: original code is words, now changed to sents
...
def get_top_n_labelled_sents(labelled_sents, n):
    sentences = processFile(labelled_sents)     
    ...
    query = buildQuery(sentences, TF_IDF_w, NUM_QUERY_WORDS)
    ...
    summary = makeSummary(sentences, best1sentence, query, n, LAMBDA, IDF_w)
...
if __name__=='__main__':
    # NOTE: changed to longer to adapt to our method
    NUM_QUERY_WORDS = 8 # originally is 8
    LAMBDA = 0.9
    ...
    for i in tqdm(range(len(corpus))):
        ...
        # to use other sentences if not enough sentences with the specified label
        label_counter = Counter(label_list)
        makeup_sent_count = 0
        other_sents = sentences.copy()
        sent_dict = {}
        for label in label_counter.keys():
            count = label_counter[label]
            label_filtered_sents = [item[0] for item in sample if item[1]==label]
            if len(label_filtered_sents) == 0: # zero labelled sent found, use other_sents to makeup
                makeup_sent_count += count
                sent_dict[label] = []
            elif len(label_filtered_sents) == 1:
                sent_dict[label] = selected_sents
                other_sents.remove(label_filtered_sents[0])
                makeup_sent_count += count - len(label_filtered_sents)
            else:
                if len(label_filtered_sents) < count:
                    makeup_sent_count += count - len(label_filtered_sents)
                selected_sents = get_top_n_labelled_sents(label_filtered_sents, min(len(label_filtered_sents),count))
                sent_dict[label] = selected_sents
                for x in selected_sents: # avoid repetition
                    other_sents.remove(x)

        if makeup_sent_count > 0:
            makeup_sents = get_top_n_labelled_sents(other_sents, makeup_sent_count)


        control_selected_summary = []
        for label in label_list:
            if len(sent_dict[label])>0:
                control_selected_summary.append(sent_dict[label][0])
                sent_dict[label] = sent_dict[label][1:]
            else:
                control_selected_summary.append(makeup_sents[0])
                makeup_sents = makeup_sents[1:]
