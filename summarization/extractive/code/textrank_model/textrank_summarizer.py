...
# src: https://tedboy.github.io/nlps/_modules/gensim/summarization/summarizer.html
...
def summarize_corpus(corpus, ratio=0.2):
    ...
    if len(corpus) < INPUT_MIN_LENGTH:
        logger.warning("Input corpus is expected to have at least " + str(INPUT_MIN_LENGTH) + " documents instead of " + str(len(corpus)))
...
def summarize(text, ratio=0.2, word_count=None, split=False):
    ...
    # NOTE: I changed text to be list of sents
    sentences = None
    for x in text:
        if sentences is None:
            sentences = _clean_text_by_sentences(x)
        else:
            sentences += _clean_text_by_sentences(x)
    ...
    if len(sentences) < INPUT_MIN_LENGTH:
        logger.warning("Input text is expected to have at least " + str(INPUT_MIN_LENGTH) + " sentences instead of " + str(len(sentences)) )
    ...
    # undo this to get original order
